...
//  TinkoffCalculator
//
//  Created by ilya Sergeev on 22.02.2024.
//
...
extension CalculationHistoryItem: Codable {
    ...
    func encode(to encoder: Encoder) throws {
        ...
        case .dateNow(let value):
            try container.encode(value.description, forKey: CodingKeys.dateNow)
        }
    }
    ...
    init(from decoder: Decoder) throws {
        ...
        if let rawOperayion = try container.decodeIfPresent(String.self, forKey: .operation),
           let operation = Operation(rawValue: rawOperayion) {
